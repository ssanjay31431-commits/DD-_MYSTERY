    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#0f0c1b]">
      ...
      <main className="flex-1 p-4 sm:p-8 space-y-6 overflow-y-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-purple-500/20">
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-white font-display">Inventory & Raw Materials</h1>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan="7" className="p-8 text-center text-slate-500 font-bold">
                    No inventory materials recorded yet. Click "Add Inventory Item" to start tracking stock.
                  </td>
                </tr>
              ) : (
                items.map((item) => (
                  <tr key={item._id} className="hover:bg-slate-900/50 transition-colors">
                    ...
                    <td className="p-4 font-bold text-amber-300">{item.quantity} {item.unit || 'pcs'}</td>
                    <td className="p-4 text-slate-400">{item.lowStockThreshold} {item.unit || 'pcs'}</td>
                    <td className="p-4">
        ...
        <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Add Inventory Material">
          <form onSubmit={handleAdd} className="space-y-4 text-xs text-slate-300">
            <div>
              ...
              <input type="text" required placeholder="Assorted Chocolates Pack" value={itemName} onChange={(e) => setItemName(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-white focus:border-pink-500 focus:outline-none" />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block mb-1 font-semibold">SKU Code</label>
                <input type="text" required placeholder="SKU-CHOC-02" value={sku} onChange={(e) => setSku(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-white uppercase font-mono" />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block mb-1 font-semibold font-semibold">Low Stock Threshold</label>
